...
import CharcterCardText from '../components/CharacterCardText';
...
const CharacterDetail = ({ route, navigation }) => {
  ...
  const goToEpisodeDetail = (id) => {
    navigation.navigate("EpisodeDetail", {itemId: id})
  }
              ...
              <FlatList
                ...
                renderItem={({ item }) => ( 
                  <View style={styles.episodeContainer}>
                    <Text style={styles.episodeText} onPress={() => goToEpisodeDetail(item.split("episode/")[1])}>{item.split("episode/")[1]}</Text>
                  </View>
                )}
              />
            </LinearGradient>
